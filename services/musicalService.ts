...
async function analyzeImage(imageUrl: string): Promise<ImageAnalysis> {
  return new Promise((resolve) => {
    if (typeof window === 'undefined') return resolve({ brightness: 0.5, hue: 0, complexity: 0.5, samples: [0.5] });
...
export const musicService = {
  start: async (imageUrl: string, style: string, theme: string) => {
    if (imageUrl === lastAnalyzedUrl) return;
    ...
    if (!audioCtx) {
      ...
      if (ContextClass) audioCtx = new ContextClass();
    }
    if (!audioCtx) return;
