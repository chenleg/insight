...
import { generateGameRound, expandPool, SUBSEQUENT_BATCH_SIZE, MAX_PREFETCH } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleImageError = () => {
    console.error("Puzzle image failed to load.");
        ...
        {gameState.status === GameStatus.IDLE && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-6">
            <div className="p-1 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 shadow-2xl shadow-indigo-500/20">
              <img src="https://picsum.photos/seed/insight/400/400" className="w-48 h-48 md:w-64 md:h-64 object-cover rounded-xl" alt="Game Preview" crossOrigin="anonymous" />
            </div>
        ...
        {(gameState.status === GameStatus.PLAYING || gameState.status === GameStatus.WON || gameState.status === GameStatus.LOST) && gameState.puzzle && (
          <div className="animate-fade-in space-y-1 md:space-y-6">
              ...
              <div className="relative w-full h-full rounded-2xl overflow-hidden bg-slate-900 border border-slate-700 shadow-2xl">
                {imageLoadError ? (
                  <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center space-y-4 bg-slate-900">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-rose-500 opacity-50"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                    <p className="text-slate-400 text-sm">The AI vision was momentarily obscured.</p>
                    <Button variant="secondary" onClick={startNewGame}>Try Another Round</Button>
                  </div>
                ) : (
                  <img 
            ...
            <div className="text-center max-w-2xl mx-auto space-y-1.5 md:space-y-6">
              ...
              {(gameState.status === GameStatus.WON || gameState.status === GameStatus.LOST) && (
                <div className={`p-4 md:p-8 rounded-2xl space-y-4 shadow-lg mx-2 md:mx-0 ${gameState.status === GameStatus.WON ? 'bg-emerald-500/10 border border-emerald-500/30 shadow-emerald-500/5 animate-bounce-slow' : 'bg-rose-500/10 border border-rose-500/30 shadow-rose-500/5'}`}>
                  {gameState.status === GameStatus.WON ? (
                    ...
                    <h2 className="text-xl md:text-3xl font-black text-rose-400 uppercase tracking-widest">Out of Attempts</h2>
                  )}
                  <p className="text-slate-300 text-xs md:text-base">The word was: <span className={`${gameState.status === GameStatus.WON ? 'text-emerald-400' : 'text-white'} font-bold tracking-widest uppercase`}>{gameState.puzzle.target_word_hidden}</span></p>
                  <Button 
                    onClick={startNewGame} 
                    variant={gameState.status === GameStatus.WON ? 'primary' : 'danger'}
                    className={`mx-auto px-8 md:px-12 py-3 md:py-4 text-base md:text-lg ${gameState.status === GameStatus.WON ? 'bg-emerald-600 hover:bg-emerald-500' : ''}`}
                  >
                    Next Challenge
                  </Button>
                </div>
              ...
              <div className="flex gap-4 overflow-x-auto pb-8 px-4 -mx-4 no-scrollbar scroll-smooth snap-x snap-mandatory">
                {gameState.history.map((item, i) => (
                  ...
                  >
                    ...
                    `}>
                      <img src={item.puzzle.image_url} className="w-full h-full object-cover" alt="Past Riddle" crossOrigin="anonymous" />
